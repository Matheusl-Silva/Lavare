...
function Icon({ name, size = 20, stroke = 1.75, className = '', style = {} }) {
  ...
  return <i data-lucide={name} style={{ width: size, height: size, strokeWidth: stroke, ...style }} className={className} />;
}
...
function Sidebar({ active }) {
  ...
  return (
    <aside className="lv-sidebar">
      <div className="lv-sidebar__brand">
        <img src="../../assets/logo.svg" alt="Lavare" height="28" />
      </div>
  ...
  return (
    <header className="lv-header">
      <div className="lv-header__search">
        ...
        <input placeholder="Buscar movimentação, item, placa…" />
        <kbd>⌘K</kbd>
      </div>
      <div className="lv-header__right">
        <button className="lv-icon-btn" aria-label="Notificações"><Icon name="bell" size={18} /></button>
